...
(function () {
  ...
  function monta(box, opz) {
    ...
    box.id = box.id || "ufbar";
    box.innerHTML =
      '<div class="uftitolo">Formazioni ufficiali dalla partita con dati ESPN</div>' +
      '<div class="ufriga">' +
        '<div class="field" style="flex:0 0 auto"><label for="ufComp">Competizione</label>' +
          '<select id="ufComp" style="width:230px"></select></div>' +
        '<div class="field" style="flex:0 0 auto"><label for="ufData">Giorno</label>' +
          '<input type="date" id="ufData" style="width:160px"></div>' +
        '<div class="field" style="flex:1 1 260px;min-width:220px"><label for="ufMatch">Partita</label>' +
          '<select id="ufMatch"></select></div>' +
        '<button id="ufCarica" type="button">&#8681; Prendi le formazioni</button>' +
        '<div class="ufnota" id="ufNota"></div>' +
      '</div>';
    ...
    function cercaPartite() {
      ...
      elMatch.innerHTML = '<option value="">…</option>';
      ...
      fetch(API + "/" + comp.code + "/scoreboard?dates=" + giorno)
        ...
        .then(function (d) {
          ...
          partite = (d.events || []).map(function (e) {
            ...
            // sembra il contrario (DOR @ HEB pare il Dortmund in casa).
            // Qui si scrive CASA - TRASFERTA, come si legge in Italia.
          ...
          nota(partite.length + " partite. Scegline una e premi <b>Prendi le formazioni</b>.");
        })
        .catch(function (err) {
          if (mio !== giro) return;
          elMatch.innerHTML = '<option value="">—</option>';
    ...
    function carica() {
      ...
      nota("Chiedo le formazioni a ESPN&hellip;");
      fetch(API + "/" + comp.code + "/summary?event=" + encodeURIComponent(id))
        ...
        .catch(function (err) {
          nota("<b>Non sono riuscito a leggerle:</b> " + esc(err.message), "err");
        });
    }
